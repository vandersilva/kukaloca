...
(function () {
    ...
    function Service($http, $q) { 
        ...
        function GetTrainings() {
            if (console) console.log("About to execute GetTrainings() inside TraininService.GetTrainings()");
            return $http.get('/api/trainings/getTrainings').then(handleSuccess, handleError);
        }

        function GetTraining(_id) {
            if (console) console.log("About to execute GetTraining() inside TraininService.GetTraining()");
            return $http.get('/api/trainings/getTraining/' + _id).then(handleSuccess, handleError);
        }

        function SaveNewTraining(training) {
            if (console) console.log("About to execute SaveNewTraining() inside TraininService.SaveNewTraining()");
            return $http.post('/api/trainings/saveNewTraining', training).then(handleSuccess, handleError);
        }

        function DeleteTraining(_id) {
            if (console) console.log("About to execute DeleteTraining() inside TraininService.DeleteTraining() for _id: %s", _id);
            return $http.delete('/api/trainings/deleteTraining/'  + _id).then(handleSuccess, handleError);
        }

        function UpdateTraining(_id, training) {
            if (console) console.log("About to execute UpdateTraining() inside TraininService.UpdateTraining() for _id: %s", _id);
        ...
        function SetIsTrained(_id, isTrained) {
            if (console) console.log("About to execute SetIsTrained() inside TraininService.SetIsTrained() for _id: %s", _id);
